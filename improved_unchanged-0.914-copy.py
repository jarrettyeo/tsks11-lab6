...
PROPORTION_OF_PREDICT_VS_SIM_LIST = [(1,1)]
...
if rerun_distance_matrix == 'y':
    run_multi_processing = input("Run multi_processing? (y/n) ") or 'n'
    assert rerun_distance_matrix in ['y','n']
...
if rerun_distance_matrix == 'y':
    ...
    if run_multi_processing == 'y':
        from multiprocessing.dummy import Pool
        from multiprocessing import cpu_count
        l = list(range(max_movie))
        with Pool(cpu_count() - 1) as p:
            p.map(run,l)
    else:
        for m in range(max_movie):
            run(m)

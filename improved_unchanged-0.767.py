...
def get_similarity(training_data, user, movie, neighbours):
    ...
    if sim_denominator != 0:
        ...
        return 0.0
        # return np.nan # TODO potential makes too low
...
def trial_and_error(NEIGHBOURS_NUMBER, PROPORTION_OF_PREDICT, PROPORTION_OF_SIM):
    ...
    for movie in range(max_movie):
        ...
        for user in range(max_user):
            ...
            improved_matrix[user, movie] = temp_val if max_user > U_MIN else np.nan # TODO
    ...
    histogram_data = []
    # u_indexes, m_indexes = np.nonzero(test_matrix) # TODO too low
    u_indexes, m_indexes = range(U_MIN, max_user), range(max_movie) # TODO
    for u_index, m_index in zip(u_indexes, m_indexes):
